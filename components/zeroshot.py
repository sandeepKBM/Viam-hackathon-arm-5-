...
Ported from the source ``viam_5`` repo's ``components/zeroshot.py`` (the
``zero-shot-detection`` branch) into color-sort's ``voice-uq`` worktree, for
use as one of the three detectors fused by ``services/perception_service.py``
(Moondream names + OWLv2 open-vocab boxes/scores + SAM masks/yaw).

You give it *text* prompts ("red block", "cup") instead of tuning HSV
ranges, and it returns boxes with no task-specific training. Output is a list
of ``DetectedShape`` (the same dataclass the OpenCV/color path produces, now
merged with a ``score`` field -- see ``components/shapes.py``) so the
existing 3D deprojection + pick pipeline works unchanged, and so the
perception service can read each detection's confidence straight off
``DetectedShape.score``.

Default model is OWLv2 (Google), a solid open-vocabulary detector. Grounding
DINO is also supported through the same ``zero-shot-object-detection``
pipeline.

Heavy deps (torch, transformers, pillow) are imported lazily (inside
``_ensure_pipeline`` / ``detect``) so importing this module never breaks an
environment that only has the OpenCV/Moondream path installed. Install them
with:

    pip install torch transformers pillow accelerate

PORT NOTE: the source repo kept the tuned default candidate-label list in a
sibling ``components/prompts.py`` (shared with its ``vlm.py`` Moondream
wrapper). This worktree is color-sort's ``voice-uq`` stack, which doesn't
have ``vlm.py`` or ``prompts.py`` (it uses ``components/moondream_client.py``
+ the ``moondream/`` server instead), and this port is scoped to exactly
``components/zeroshot.py`` -- so rather than introduce a new shared module,
the same tuned default prompts are inlined below as ``_DEFAULT_ZS_PROMPTS``.
``ZS_PROMPTS`` env var override still works exactly as before.
"""
...
# Tuned candidate labels for the real tabletop set this stack targets (red/
# yellow block, cup/mug, pen, soda can/can) -- inlined from the source repo's
# components/prompts.py (see PORT NOTE above). canonicalize.py folds every
# variant (e.g. "mug", "soda can") back onto color-sort's pick vocabulary.
_DEFAULT_ZS_PROMPTS: tuple[str, ...] = (
    "red block",
    "yellow block",
    "cup",
    "mug",
...
class ZeroShotDetector:
    ...
    def _ensure_pipeline(self):
        ...
        if self._device is None:
            # cuda (server) -> mps (Apple Silicon MacBook) -> cpu. The old code
            # only checked cuda, so on a Mac OWLv2 silently fell to CPU; prefer
            # MPS there. ZS_DEVICE overrides (e.g. "cpu", "mps", "cuda:0").
            env_device = os.environ.get("ZS_DEVICE", "").strip()
            if env_device:
                device = env_device
...
async def locate_objects_zeroshot(
    ...
    using the same depth + intrinsics helpers as the color pipeline. Needs a
    live Viam camera (``machine``) -- not used by the image-only perception
    service, which calls ``detect_objects`` directly on an already-decoded
    frame instead, but kept here so on-robot callers (sort_blocks.py-style
    scripts) still have it after the port.
    """
